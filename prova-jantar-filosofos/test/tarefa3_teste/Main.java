...
public class Main {
    ...
    public static void main(String args[]) {
        ...
        // Tempo de execução configurado para 5 minutos (300 segundos)
        ...
        // 2. Cria o Semáforo com N-1 permissões (4)
        Semaphore limiteFilosofos = new Semaphore(NUM_FILOSOFOS - 1); 

        // 3. Criar os Filósofos, atribuir Garfos e Semáforo
        for (int i = 0; i < NUM_FILOSOFOS; i++) {
            Garfo garfoEsquerdo = garfos[i];
            Garfo garfoDireito = garfos[(i + 1) % NUM_FILOSOFOS];

            Filosofo f = new Filosofo("F" + String.valueOf(i + 1), garfoEsquerdo, garfoDireito, limiteFilosofos);
            filosofos.add(f); 
        }
        ...
        // 6. Exibir Estatísticas - Passando NUM_FILOSOFOS
        exibirEstatisticas(filosofos, duracaoExecucaoMs, "Semáforo N-1 (T3)", NUM_FILOSOFOS);
    }
